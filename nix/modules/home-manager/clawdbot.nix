...
let
  ...
  firstPartySources =
    let
      stepieteRev = "c83cd1bbe0fc9a24c48046598bd9ce817e839328";
      stepieteNarHash = "sha256-c/PTqiZ/QeZN1FGE1a7//nHzkxfw0n+pPKpwQ77BBj4=";
      stepiete =
        tool:
        "github:joshainglis/nix-steipete-tools?dir=tools/${tool}&rev=${stepieteRev}&narHash=${stepieteNarHash}";
    in
    {
  ...
  resolvePlugin =
    ...
    let
      ...
      clawdbotPlugin =
        if flake ? clawdbotPlugins then
          flake.clawdbotPlugins.${system} or null
        else
          flake.clawdbotPlugin or null;
      needs = if clawdbotPlugin != null then clawdbotPlugin.needs or { } else { };
    in
    if clawdbotPlugin == null then
      null
    else
      {
  ...
  resolvedPluginsByInstance = lib.mapAttrs (
    instName: inst:
    let
      resolved = lib.filter (p: p != null) (map resolvePlugin inst.plugins);
